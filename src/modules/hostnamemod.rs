...
use crate::configs::hostnamemod::HostnamemodConfig;
...
pub fn module<'a>(context: &'a Context) -> Option<Module<'a>> {
    let mut module = context.new_module("hostnamemod");
    let config: HostnamemodConfig = HostnamemodConfig::try_load(module.config);
    ...
    let parsed = StringFormatter::new(config.format).and_then(|formatter| {
        formatter
            ...
            .map_style(|variable| match variable {
                "style" => {
                    let module_style = if ssh_connection.is_some() {
                        config.style_ssh
                    } else {
                        config.style_local
                    };
