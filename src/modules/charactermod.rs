...
use crate::configs::charactermod::CharactermodConfig;
...
pub fn module<'a>(context: &'a Context) -> Option<Module<'a>> {
    ...
    let mut module = context.new_module("charactermod");
    let config: CharactermodConfig = CharactermodConfig::try_load(module.config);
    ...
    let style = match mode {
        ShellEditMode::Normal => {
            if is_root {
                config.vimcmd_root_style
            } else {
                config.vimcmd_normal_style
            }
        },
        ShellEditMode::Visual => config.vimcmd_visual_style,
        ShellEditMode::Replace => config.vimcmd_replace_style,
        ShellEditMode::ReplaceOne => config.vimcmd_replace_one_style,
    ...
    let symbol = if mode == ShellEditMode::Insert {
        if is_root {
            config.root_symbal
        } else {
            config.normal_symbal
        }
    } else {
        if is_root {
            config.root_vimcmd_symbal
        } else {
            config.normal_vimcmd_symbal
        }
    };
